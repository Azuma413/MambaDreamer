...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        with_dconv = (self.d_conv > 1) # 畳み込みがあるかどうか
        x = self.in_proj(x) # 全結合層 初期投影
        if not self.disable_z:
            x, z = x.chunk(2, dim=-1) # テンソルを分割
            if not self.disable_z_act:
                z = self.act(z) # 半分は活性化関数に通す
        if with_dconv:
            x = x.permute(0, 3, 1, 2).contiguous() # テンソルの次元を入れ替える
            x = self.conv2d(x) # (b, d, h, w) 畳み込み層
        x = self.act(x) # 活性化関数
        y = self.forward_core(x, channel_first=with_dconv) # forward_coreはforward_corev2。
        if not self.disable_z:
            y = y * z
...
class VSSM(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        x = self.patch_embed(x) # 画像を4*4分割して埋め込む
