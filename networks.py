...
from mamba_simple import MambaCell # 公式のmambaをもとに作成したモジュール
import tools
...
class RSSM(nn.Module):
    ...
    def initial(self, batch_size):
        ...
        if self._initial == "zeros":
            ...
            state["deter"] = torch.tanh(self.W).repeat(batch_size, 1) # deter: 
    ...
    def obs_step(self, prev_state, prev_action, embed, is_first, sample=True):
        ...
        x = self._obs_out_layers(x) # xを全結合層に通して，hidden次元の特徴量を得る
        # (batch_size, hidden) -> (batch_size, stoch, discrete_num)
        stats = self._suff_stats_layer("obs", x) # DreamerV3では離散分布の確率的状態の潜在表現を得る(posterior)
        if sample: # サンプリングする場合
            stoch = self.get_dist(stats).sample() # 確率分布に基づきサンプリング
        else: # サンプリングしない場合
            stoch = self.get_dist(stats).mode() # 確率分布の最頻値を取得
        post = {"stoch": stoch, "deter": prior["deter"], **stats} # 1ステップ先の観測の情報を取り込んで計算した状態表現である"posterior"を得る
    ...
    def img_step(self, prev_state, prev_action, sample=True):
        ...
        prev_stoch = prev_state["stoch"] # 全結合層により学習された環境の確率的な状態の潜在表現
        if self._discrete: # Dreamerではガウス分布を用いていたが，DreamerV2からは離散分布を用いている
            shape = list(prev_stoch.shape[:-2]) + [self._stoch * self._discrete]
            # (batch, stoch, discrete_num) -> (batch, stoch * discrete_num)
            prev_stoch = prev_stoch.reshape(shape)
        # (batch, stoch * discrete_num) -> (batch, stoch * discrete_num + action)
        x = torch.cat([prev_stoch, prev_action], -1) # 前の確率的状態の潜在表現と前の行動を結合
        # (batch, stoch * discrete_num + action, embed) -> (batch, hidden)
        x = self._img_in_layers(x) # 確率的状態と行動を結合した配列を全結合層に通して，hidden次元の特徴量を得る
        for _ in range(self._rec_depth):  # rec depth is not correctly implemented
            deter = prev_state["deter"] # 決定的な状態の潜在表現をdeterに格納
            # (batch, hidden), (batch, deter) -> (batch, deter), (batch, deter)
            # if self._use_mamba:
            #     print("call Mamba cell")
            # else:
            #     print("call GRU cell")
            x, deter = self._cell(x, [deter]) # _cellはGRUCellクラスのインスタンス。隠れ層のxとdeterを入力として，次の決定的状態(rnn_hidden)を得る。実装を見るとxとdeterは同じ値を持っている return x, [x]
            deter = deter[0]  # Keras wraps the state in a list.
        # (batch, deter) -> (batch, hidden)
        x = self._img_out_layers(x) # x=deterを全結合層に通して，hidden次元の特徴量を得る
        # (batch, hidden) -> (batch_size, stoch, discrete_num)
        stats = self._suff_stats_layer("ims", x) # Dreamerではガウス分布を計算していた部分。DreamerV3では離散分布を計算して新たな確率的状態の潜在表現を得る
        if sample:
            stoch = self.get_dist(stats).sample() # 確率分布に基づきサンプリング
        else:
            stoch = self.get_dist(stats).mode() # 確率分布の最頻値を取得
        prior = {"stoch": stoch, "deter": deter, **stats} # 状態遷移を用いた1ステップ先の未来の状態表現である"prior"を得る
        return prior
    ...
    def _suff_stats_layer(self, name, x):
        """
        状態遷移を用いた1ステップ先の未来の状態表現である"prior"から状態空間を推定する場合はimsを指定  
        1ステップ先の観測の情報を取り込んで計算した状態表現である"posterior"から状態空間を推定する場合はobsを指定  
        この2種類の状態表現を用いて学習を行う  
        """
        if self._discrete:
            if name == "ims":
                x = self._imgs_stat_layer(x) # _imgs_stat_layerも_obs_stat_layerも同じ構造を持った全結合層 hidden -> stoch*discrete
            elif name == "obs":
                x = self._obs_stat_layer(x) # 離散的な状態表現を取得
            else:
...
class MultiEncoder(nn.Module):
    """
    CNNを用いて画像をエンコードする
    入力がベクトルの場合はMLPを用いて入力ベクトルをエンコードする(DreamerV3では必要ない)
    """
    def __init__(
        self,
        shapes,
...
class MultiDecoder(nn.Module):
    """
    CNNを用いて画像を生成する
    入力がベクトルの場合はMLPを用いて元のベクトルを復元する(DreamerV3では必要ない)
    """
    def __init__(
        self,
        feat_size,
