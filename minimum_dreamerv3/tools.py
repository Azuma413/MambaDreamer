...
def lambda_return(reward, value, pcont, bootstrap, lambda_, axis):
    ...
    value: 状態価値の配列
    pcont: 割引率の配列
    bootstrap: ブートストラップの値?
    lambda_: TD(lambda)のlambda どれだけ遠い未来の報酬を考慮するか
    """
...
class Optimizer:
    ...
    def __call__(self, loss, params, retain_graph=True):
        ...
        metrics = {}
        metrics[f"{self._name}_loss"] = loss.detach().cpu().numpy() # lossをcpuに移してnumpyに変換
        self._opt.zero_grad() # pytorchは勾配を累積させるため，毎回勾配を初期化する必要がある
        self._scaler.scale(loss).backward(retain_graph=retain_graph) # 同じ計算グラフを使って勾配を計算するため，retain_graph=True。誤差逆伝播を行い，勾配を計算
        self._scaler.unscale_(self._opt) # 勾配のスケールを戻す
        # loss.backward(retain_graph=retain_graph)
        norm = torch.nn.utils.clip_grad_norm_(params, self._clip) # 勾配爆発を防ぐため，勾配をクリップする
        if self._wd:
            self._apply_weight_decay(params) # 過学習を防ぐため，重み減衰を適用する
        self._scaler.step(self._opt) # パラメータを更新する
        self._scaler.update() # スケーラーを更新する
        # self._opt.step()
        self._opt.zero_grad()
